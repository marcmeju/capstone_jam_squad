...
public class BookingJdbcTemplateRepository implements BookingRepository{
    ...
    public Booking add(Booking booking) {
        final String sql = "insert into Booking (package_id, customer_id, num_of_guest) values (?, ?, ?);";
        ...
        int rowsAffected = jdbcTemplate.update(connection -> {
            ...
            ps.setInt(3, booking.getNumOfGuest());
    ...
    public boolean update(Booking booking) {
        final String sql = "update Booking set package_id = ?, customer_id = ?, num_of_guest = ?  where booking_id = ?;";

        int rowsUpdated = jdbcTemplate.update(sql,
                booking.getPackageId(), booking.getCustomerId(), booking.getNumOfGuest(), booking.getBookingId());
